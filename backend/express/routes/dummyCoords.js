...
router.get("", (req, res) => {
  dummyJson
    .find()
    .then((dummyData) => {
      res.status(200).json(dummyData[0]);
    })
    .catch((error) => {
      res.status(401).json({
        error: error,
  ...
  for (i = 0; i < req.body.features.length; i++) {
    let feature = {
      ...
      geometry: {
        ...
        coordinates: [
          req.body.features[i].geometry.coordinates[0],
          req.body.features[i].geometry.coordinates[1],
          req.body.features[i].geometry.coordinates[2],
        ],
      },
    };
